...
export default function LogHours() {
  ...
  useEffect(() => {
    async function loadStudents() {
      ...
      const uniqueStudents = Array.from(
        new Map(
          (data || []).map((item: any) => [item.student_id, item])
        ).values()
      );

      setStudents(uniqueStudents);
      if (uniqueStudents.length > 0) {
        setStudentId(uniqueStudents[0].student_id);
      }
      setLoading(false);
    }
  ...
  const handleLogHours = async (e: React.FormEvent) => {
    ...
    if (!studentId || !date || !hours) {
      toast.error("Please select a student, date, and hours.");
      return;
    }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="space-y-10">
      <header>
        ...
        <p className="text-stone-500 mt-2">Record every session directly to Supabase so your dashboard metrics update automatically.</p>
      </header>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                placeholder="Algebra, Writing, Science"
                className="w-full bg-stone-100 border-none rounded-2xl p-4 outline-none focus:ring-2 focus:ring-black transition-all"
              />
            </div>
          </div>
