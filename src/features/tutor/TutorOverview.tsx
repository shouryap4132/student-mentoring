...
export default function TutorOverview({ profile }: any) {
  ...
  const [metrics, setMetrics] = useState({ volunteeredHours: 0, rating: 5.0, activeMentees: 0 });
  ...
  useEffect(() => {
    async function fetchDashboardData() {
      ...
      const { data: meetingData } = await supabase
        .from("requests")
        .select(`meeting_date, student:profiles!requests_student_id_fkey(full_name)`)
        .eq("tutor_id", tutorId)
        .eq("status", "scheduled")
      ...
      const { data: logRows } = await supabase.from("hours_logs").select("hours").eq("tutor_id", tutorId);
      const volunteeredHours = (logRows || []).reduce((sum: number, row: any) => sum + Number(row.hours || 0), 0);

      setMetrics({ volunteeredHours, rating: profile?.avg_rating || 5.0, activeMentees: uniqueMentees });
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      requestsChannel = supabase
        ...
        .on("postgres_changes", { event: "*", schema: "public", table: "requests", filter: `tutor_id=eq.${session.user.id}` }, fetchDashboardData)
        .subscribe();
      hoursChannel = supabase
        .channel(`overview-hours-${session.user.id}`)
        .on("postgres_changes", { event: "*", schema: "public", table: "hours_logs", filter: `tutor_id=eq.${session.user.id}` }, fetchDashboardData)
        .subscribe();
    });
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <MetricCard label="Total Volunteered" value={`${metrics.volunteeredHours} hrs`} sub="Lifetime impact" />
        <MetricCard label="Student Rating" value={metrics.rating.toFixed(1)} sub="Based on student feedback" />
        <MetricCard label="Active Mentees" value={metrics.activeMentees.toString()} sub="Currently mentoring" />
      </div>
          ...
          <div className="text-center md:text-right">
            ...
            {loading ? (
              ...
              <div>
                ...
                <p className="text-xs font-medium text-stone-500">with {nextMeeting.student?.full_name}</p>
              </div>
            ) : (
