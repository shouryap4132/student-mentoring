...
export default function ProgressTracking({ profile }: any) {
  const [progress, setProgress] = useState({
    hoursLearned: 0, sessionsAttended: 0, goalHours: profile?.goal_hours || 30, loading: true,
  });
  ...
  useEffect(() => {
    async function fetchProgress() {
      ...
      setProgress({ hoursLearned: totalHours, sessionsAttended: data?.length || 0, goalHours: profile?.goal_hours || 30, loading: false });
    }
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      channel = supabase
        ...
        .on("postgres_changes",
          { event: "*", schema: "public", table: "hours_logs", filter: `student_id=eq.${session.user.id}` },
          () => fetchProgress())
        .subscribe();
    });
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-black text-white p-10 rounded-[3rem]">
        ...
        <h4 className="text-2xl font-playfair mb-8">
          {progress.loading ? "Loading progress..." : `${percentage.toFixed(0)}% completed`}
        </h4>
